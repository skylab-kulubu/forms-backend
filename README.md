...
- **Keycloak prerequisite (audience):** The API is the Keycloak client `forms`, but people sign in through other clients (SkyForms frontend: `skyforms`). Keycloak puts `forms` into a user's `aud` only through an explicit **Audience** mapper on the login client or, via the built-in *audience resolve* mapper, when the token already carries `forms` client roles. Admins therefore pass without any mapper while plain members get `401 error="invalid_token", error_description="The audience ... is invalid"` on every call. Every login client that obtains user tokens for this API must carry an Audience mapper with *Included Client Audience* `forms` (production and sandbox realms): for `skyforms` that is the client scope `skyforms-forms-audience` (mapper `forms-audience`, access token on, ID token off) attached as a **Default** client scope. Do not relax `Authentication__Audience` to work around a missing mapper; fix the realm.
...
| `Authentication__Audience` (Compose: `AUTH_AUDIENCE`) | Incoming-token audience; startup rejects every value except `forms`. Login clients must put `forms` into `aud` through an Audience mapper (see *Authentication & Authorization*) | No, fixed default |
